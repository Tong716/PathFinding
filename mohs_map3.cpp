...
bool load_edge_txt_file(std::string txt_file, std::vector<Edge> &edges_out, size_t &graph_size, int num_objectives) {
    ...
    while (std::getline(file, line)) {
        ...
        if (decomposed_line.size() < (size_t)num_objectives + 2) {
            continue;
        }
        ...
        for (size_t i = 2; i < decomposed_line.size() && (int)costs.size() < num_objectives; ++i) {
...
std::string generate_output_header(int num_objectives) {
    std::ostringstream ss;
    ss << "solver,src,dest";
    if (num_objectives == 3) {
        ss << ",distance,travel_time,elevation";
    } else if (num_objectives == 4) {
        ss << ",distance,travel_time,elevation,avg_degree";
    } else {
        for (int j = 0; j < num_objectives; j++) {
            ss << ",cost" << (j + 1);
        }
    }
    ss << ",path,runtime,num_generation,num_expansion\n";
    return ss.str();
}
...
void single_run_map(AdjacencyMatrix& graph, AdjacencyMatrix&inv_graph, size_t source, size_t target, std::ofstream& output, std::string algorithm , double eps, unsigned int time_limit, po::variables_map& vm) {
    ...
    int num_objectives = graph.get_num_of_objectives();
    if (solutions.empty()) {
...
void run_query(AdjacencyMatrix & graph, AdjacencyMatrix & inv_graph, std::string query_file, std::string output_file, std::string algorithm, double eps, int time_limit, po::variables_map& vm) {
    ...
    if (!file_exists) {
        stats << generate_output_header(graph.get_num_of_objectives());
    }
...
int main(int argc, char** argv){
    ...
    desc.add_options()
        ...
        ("num-objectives", po::value<int>()->default_value(4), "number of objective dimensions to load from edge files") ;
    ...
    if (datasetRoot != "") {
        ...
        int numObjectives = vm["num-objectives"].as<int>();
        ...
        for (const auto& region : regions) {
            ...
            if (!load_edge_txt_file(edgeFile, edges, graphSize, numObjectives)) {
            ...
            for (const auto& alg : algorithms) {
                std::string outFile = outputDir + "/" + region + "_" + alg + "_" + std::to_string(numObjectives) + "obj.txt";
